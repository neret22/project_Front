...
import FoundPetCard from "../FoundAndLost/FoundPetCard";
...
const AllPosts = () => {
    ...
    const [foundPosts, setFoundPosts] = useState()
     ...
     useEffect( () => {
            setTimeout( () => {
                fetch(`http://propets.telran-edu.de:8080/api/v1/posts`)
    ...
    // const getFoundPostsById = (id) => {
    //     const idx = posts.filter(post => post.userId === activeUser.id)
    //     if (idx === -1) {
    //         return null
    //     }
    ...
    // }
        const renderFoundPosts = () => {
            if (isLoading && !error) {
                ...
                return (foundPosts.filter((post) => post.userId === activeUser.id).map((filteredPost) => {
                    return (
                        <FoundPetCard key={foundPost.userId} foundPet={foundPost}/>
                    )
                }))
        }
